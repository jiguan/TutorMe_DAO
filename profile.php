...
$username = ! empty ( $_GET ['tutorid'] ) ? $_GET ['tutorid'] : $_SESSION ['username'];
}
?>
</head>
<body>
		...
		<div class="container" style="height: 1000px;">
			...
			<div id="userScheduleDiv" style="position: relative"
				...
				<div id="generalInfo">
					...
					<div id="viewInfo">
						<?php populateViewInfoForm($username)?>
					</div>
				...
				$row = $tutorDAO->load($username);
					...
					populateCourseAndRatings ( $username );
					echo "</div>";
					echo "<div id='editCourses' style='margin-left:200px;display: none;'>";
					populateEditCourseView ( $username );
					echo "</div>";
					echo "<br />";
					...
					$result = $TimeTableDAO->queryByUsername($username);
					$data = '';
					for($i=0;$i<count($result);$i++) {
						...
						$data .= '
							        	data[' . $i . '] = {
									Date: "' . $row2->date . '",
							              StartTime: "' . $row2->beginTime . '",
									EndTime: "' . $row2->endTime . '"
							        };
							    ';
					}
					
					if (isEditable ( $username ) == true) {
						echo "<a href='tutorAddTime.php'>Add new...</a>";
					}
