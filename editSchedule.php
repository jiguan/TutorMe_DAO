...
<?php
	...
	$row = $scheduleDAO->load($scheduleID);
		...
		<div class="container">
			...
			<div id="userScheduleDiv" style="position: relative"
				...
				<div class="schedule">
					...
					<br />
					<form id="schedule" method="post" action="updateSchedule.php">
...
function populateBeginTime()
{
	...
	if( tutorID.length != 0 && date.length != 0 )
	{
		...
		xmlhttp.open("GET","controller/dbRequestProcessor.php?type=getAvailableTime&date="+date+"&tutorID="+tutorID, true);
		xmlhttp.send();		
	}
}
...
function populateLocation()
{
	...
	if( beginTime.length != 0 && endTime.length != 0 && date.length != 0)
	{
		...
		xmlhttp.open("GET","controller/dbRequestProcessor.php?type=getLocation&date="+date+"&beginTime="+beginTime+"&endTime="+endTime, true);
		xmlhttp.send();		
	}
	else
	{
